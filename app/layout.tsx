...
export default function RootLayout({
    ...
    return (
        ...
        >
        <main className={"flex-col space-y-6 bg-white border-[3px] shadow-[4px_4px_0px_0px_rgba(0,_0,_0,_1)] border-black rounded-2xl " +
            ...
            <nav className={"flex justify-between items-center transition-all ease-in-out"}>
                <Link href={"/"} className={"text-7xl font-[900] flex items-center space-x-2 group"}>
                    ...
                    <Undo2 className={"text-white bg-coal p-1 rounded-sm opacity-30 group-hover:opacity-100 " +
                        "group-hover:bg-jade  transition-all ease-in-out duration-500"} />
                </Link>
                <section className={"flex items-center space-x-4"}>
                    {routes.map((route: Route) => (
                        <div className={"flex-col group"} key={route.name}>
